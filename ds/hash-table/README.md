...
Under reasonable assumptions, __the average time to search for an element in a hash table is $O(1)$__.

> Altough searching for an element in a hash table can take as long as searching for an element in a linked list (i.e., $O(n)$ time worst case), in practice, hashing performs extremely well.
...
A hash table maps keys to values. It uses a _hash function_ to figure out where to store elements.

## Implementation

### Direct-address table

When the set `K` of keys stored in a dictionary is much smaller than the universe `U` of all possible keys, a hash table __requires much less storage__ than a direct-address table. Specifically, we can reduce the storage requirements to $O(|K|)$ while we maintain the benefit that searching for an element in the hash table still requires only $O(1)$ time. The __catch__ is that this bound is for the __average-case time__, whereas for direct addressing it holds for the _worst-case time_.
...
To represent the dynamic set, we use an array, or __direct-address table__, denoted by $T[0..,m-1]$, in which each __slot__, corresponds to a key in the universe $U$. If the set contains no element with the key $k$, then $T[k] = NIL$.

Each operation takes only $O(1)$ time.

The __downside__ is that if the universe $U$ is large, storing a table of size $|U|$ may be impractical. Furthermore, the set $K$ of keys actually stored may be so small relative to $U$ that most of the space allocated for $T$ would be wasted.
...
The value of the constant $A$, depends on characteristics of the data being hashed. Knuth suggests that $\frac{\sqrt(5) - 1}{2} \approx 0.6180339887...$ is likely to work reasonably well.
...
$h:\ U\ x\ \{0, 1, ..., m-1\} \rightarrow \{0, 1, ..., m-1\}$
...
It offers one of the best methods available for _open addressing_ because the permutations produced have many of the characteristics of randomly chosen permutations. It uses a hash function of the form:
